...
dsp2.util.cluster.tmp_write( '-clear' );
...
for i = 1:numel(un_processed)
  ...
  dsp2.util.cluster.tmp_write( {'Loading %s\n', write_str} );
  loaded = io.read( pcomplete, 'only', days );
  loaded = loaded.rm( 'choice' );
  dsp2.util.cluster.tmp_write( {'Done loading %s\n', write_str} );
  sites = loaded( 'sites' );
  if ( numel(sites) > 16 )
  ...
  dsp2.util.cluster.tmp_write( {'Averaging %s\n', write_str} );
  loaded = dsp2.process.outliers.keep_non_clipped( loaded );
  coh = loaded.parfor_each( m_within, sfunc );
  dsp2.util.cluster.tmp_write( {'Done averaging %s\n', write_str} );
  fname = [ day_str, '.mat' ];
  dsp2.util.cluster.tmp_write( {'Saving %s\n', write_str} );
  save( fullfile(save_path, fname), 'coh' );
  dsp2.util.cluster.tmp_write( {'Done saving %s\n', write_str} );
end

dsp2.util.cluster.tmp_write( 'Done averaging new days\nWriting old days\n' );

for i = 1:numel(meaned_days)
  if ( dsp2.cluster.should_abort() ), break; end
  coh = io.read( pmeaned, 'only', meaned_days{i} );
  coh = coh.rm( 'choice' );
  day_str = strjoin( meaned_days{i}, '_' );
