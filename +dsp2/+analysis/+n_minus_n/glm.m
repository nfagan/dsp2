function mdl_cont = glm( n, n_minus_n, factors, varargin )

%   GLM -- Construct a GLM of a previous-trial distribution's
%     influence on a current-trial distribution's values.
%
%     mdl = ... glm( N, N_minus_2, {'outcomes', 'trialtypes'} )
...
[predicts, response, params] = prep( n, n_minus_n, factors, varargin{:} );

link = params.link;
distribution = params.distribution;

mdl = fitglm( predicts, response, 'link', link, 'distribution', distribution );
coeffs = mdl.Coefficients;
coeffs.Properties.RowNames = coeff_names;
betas = coeffs.Estimate;
